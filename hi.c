...
player_t *create_player_list(int list_count) {
    ...
    //Dynamically allocate space for player list
    player_t *player_list = malloc(sizeof(player_t)*list_count);
...
void initialize_player(player_t *player, char *name, int card_count) {
    ...
    //Dynamically allocate card count
    player->list_of_cards = malloc(sizeof(card_t)*card_count);
...
void set_player_card_info_at(player_t *player, int card_position, char suit, int rank) {
    ...
    //Set values
    player->list_of_cards[card_position].suit = suit;   
...
void display_player_information(player_t *player) {
    ...
    */
    ...
    //For loop to print every card in player's hand.
...
void cleanup(player_t *player_list, int list_count) {
    ...
    for(int i = 0; i<list_count; i++)
    {
        //Set all pointers to NULL to ensure no infomration is floating in heap
        player_list[i].name = NULL;
        player_list[i].list_of_cards = NULL;
